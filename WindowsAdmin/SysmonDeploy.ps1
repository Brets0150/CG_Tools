...
.EXAMPLE
	...
	PS> Invoke-WebRequest https://github.com/Brets0150/CG_BlueTeamTools/blob/main/SysmonDeploy.ps1 -O .\SysmonDeploy.ps1
...
[string]$global:SysmonConfig_Temp = $env:temp + "sysmon-config.xml"
...
function Install-SysmonDnsConfig() {
	...
	[string]$SysmonDNSConfig = '<Sysmon schemaversion="4.21"><EventFiltering><DnsQuery onmatch="exclude" /></EventFiltering></Sysmon>'

	# Create the Sysmon configuration file.
	Set-Content -Path $global:SysmonConfig_Temp -Value $SysmonDNSConfig  -Force
